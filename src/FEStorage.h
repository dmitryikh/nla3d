...
public:
  ...
  // storing operations
  //
  // The function stores PostProcessor in FEStorage. Then FEStorage takes care about 
  // dynamically allocated object - FEStorage will free the memory.
  ...
  // the function to apply boundary conditions
	void applyBoundaryConditions (uint16 curLoadstep, uint16 curIteration, double d_par, double cum_par);
...
private:
  ...
  // of FEStorage. But with function addPostProcessor(..) FEStorage take control on the PostProcessor instance.
  // They are deleted in ~FEStorage() destructor.
