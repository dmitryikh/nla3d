#include "ColorN_Scene.h"

void ColorN_Scene::readMesh (FE_Storage *st)
{
	...
	colors.assign(st->nn*4,-1);
	ReleaseMutex(scene_lock);
}

void ColorN_Scene::draw () 
{
	...
	for (uint32 i=0; i<points[0].size(); i++)
	{
		...
		{
			glColor3fv(cmap[colors[n_count++]].rgb);
	...
	for(uint16 i=0; i < this->cmap.colors.size(); i++)
	{
		...
		glEnd();
		glColor3d(255/255.0f,255/255.0f,255/255.0f);
...
void ColorN_Scene::setup_view (uint16 width, uint16 height)
{
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	this->width = width;
